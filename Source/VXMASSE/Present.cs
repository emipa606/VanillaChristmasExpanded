...
namespace VXMASSE
{
	...
	public class Present : Building, IThingHolder, IOpenable
	{
        ...
        public bool HasAnyContents => true;
		...
		public void EjectContents()
		{
			...
			if (flag)
			{
                ...
                ThingCategoryDef thingCategoryDef = list.RandomElement();
				IEnumerable<ThingStuffPair> source = ThingStuffPair.AllWith((ThingDef td) => td.thingCategories != null && td.thingCategories.Contains(thingCategoryDef));
				while (source.Count() < 1)
				{
					thingCategoryDef = list.RandomElement();
					source = ThingStuffPair.AllWith((ThingDef td) => td.thingCategories != null && td.thingCategories.Contains(thingCategoryDef));
				}
				ThingStuffPair thingStuffPair = source.RandomElement();
				Thing thing = ThingMaker.MakeThing(thingStuffPair.thing, thingStuffPair.stuff);
                CellFinder.TryRandomClosewalkCellNear(Position, Map, 1, out IntVec3 intVec, null);
				...
				GenPlace.TryPlaceThing(thing, intVec, Map, ThingPlaceMode.Direct, null, null);
			...
			else
			{
				...
				foreach (IncidentDef incidentDef in allDefs)
				{
					IncidentParms parms = StorytellerUtility.DefaultParmsNow(incidentDef.category, Map);
					var flag9 = incidentDef.Worker.CanFireNow(parms, false) && incidentDef.defName != "StrangerInBlackJoin" && incidentDef.defName != "PresentDrop" && incidentDef.defName != "ShortCircuit" && incidentDef.letterDef != null && incidentDef.letterDef.defName != "PurpleEvent" && !incidentDef.targetTags.Contains(IncidentTargetTagDefOf.World) && !incidentDef.targetTags.Contains(IncidentTargetTagDefOf.Caravan);
					if (flag9)
					{
						list2.Add(incidentDef);
					}
				}
				IncidentDef incidentDef2 = list2.RandomElement();
				var flag10 = incidentDef2 == null;
				if (flag10)
				{
					EjectContents();
				}
				IncidentParms incidentParms = StorytellerUtility.DefaultParmsNow(incidentDef2.category, Map);
				var pointsScaleable = incidentDef2.pointsScaleable;
				if (pointsScaleable)
				{
